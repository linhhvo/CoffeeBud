...
To verify that the database tables are set up correctly, run `docker exec -it coffeebud_database psql coffeebud_db -U <your_username>` and use command `\dt` in `psql` to list all the tables created.
