...
interface PetAvatarFieldProps {
  /** Currently saved avatar URLs fetched from the server. */
...
export function PetAvatarField ({ existing, onChange }: PetAvatarFieldProps) {
  ...
  const notifyParent = (
    mood: Mood,
    file: File | null,
    current: Record<Mood, SlotState | null>
  ) =>
    onChange?.({
  ...
  const handleFile = async (mood: Mood, raw: File) => {
    ...
    setSlots((prev) => {
      if (prev[mood]?.previewUrl) URL.revokeObjectURL(prev[mood]!.previewUrl);
      return {
        ...prev,
        [mood]: { previewUrl, file: raw, status: "processing", error: null },
      };
    });
  ...
  const handleFileInput = (mood: Mood, e: React.ChangeEvent<HTMLInputElement>) => {
  ...
  return (
    <div>
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
        {EMOTION_SLOTS.map(({ mood, label, description }) => {
          ...
          return (
            <div key={mood} className="flex flex-col gap-2">
              ...
              <div
                ...
                onDragOver={(e) => { e.preventDefault(); if (!isProcessing) setDragOver(mood); }}
              ...
              >
                {displayUrl ? (
                  <>
                    ...
                    {isProcessing && (
                      <div className="absolute inset-0 flex flex-col items-center justify-center gap-2 bg-zinc-900/60">
                        ...
                        <span className="text-[10px] text-zinc-400">Resizing & converting…</span>
                      </div>
                    )}
                      ...
                      <div className="absolute inset-0 flex flex-col items-center justify-center gap-2 bg-zinc-900/80 opacity-0 hover:opacity-100 transition-opacity duration-150">
                        ...
                        {!isExisting && (
                          <button
                            ...
                            onClick={(e) => { e.stopPropagation(); handleRemove(mood); }}
                          >
                            Remove
                          </button>
                        )}
                  ...
                  <div className="flex flex-col items-center gap-2 pointer-events-none select-none">
                    <img src={MOOD_EMOJI[mood]} alt={label} className="w-8 h-8 object-contain" />
              ...
              <div className="flex flex-col gap-0.5 px-0.5">
                ...
                {isError && slot?.error && (
                  <span className="text-xs text-red-400 mt-0.5">{slot.error}</span>
                )}
                {slot && !isError && (
                  <span className="text-[11px] text-zinc-600 truncate mt-0.5" title={slot.file.name}>
                    {slot.file.name}
                  </span>
                )}
              </div>
