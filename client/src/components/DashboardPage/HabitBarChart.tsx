...
import {type ChartDayData, type DailyStat, DAY_LABELS} from "./types.ts";
...
function CustomCursor({x, y, width, height, payload}: any) {
    if (!payload?.[0]?.payload.isMissing) return null;
    return <rect x={x} y={y} width={width} height={height} fill="rgba(255,255,255,0.03)"/>;
}

export function HabitBarChart({chart, data, target}: ChartCardProps) {
    const chartData: ChartDayData[] = data.map((d) => {
        ...
        return {
            ...
            date: d.date.toLocaleDateString("en-GB", {day: "numeric", month: "numeric"}),
            value: value ?? 0,
    ...
    return (<div
        ...
        <div className="flex flex-wrap gap-x-3 gap-y-1">
            ...
            <div className="flex items-center gap-1.5">
                <span className="inline-block w-5" style={{borderTop: "2px dashed #a78bfa", height: 0}}/> <span
        ...
        >
            ...
            />
            <Tooltip
                content={(props) => (<ChartTooltip {...props} chart={chart} target={target}/>)}
                cursor={<CustomCursor/>}/>
            <ReferenceLine
                y={target}
                stroke="#a78bfa"
